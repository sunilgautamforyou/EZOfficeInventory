...
<body>
    ...
    <header class="header">
        ...
        <div class="breadcrumb-holder">
            <div class="container-fluid">
                <ul class="breadcrumb">
                    <li class="breadcrumb-item active">Request For Quotation Home Page</li>
                </ul>
            </div>
        </div>
        <section>
...
function loadHomePGData() {
		   ...
		   data: JSON.stringify(
		   	...
		   	success: function (data) {
		        ...
		        for(var i=0;i<data.length;i++){
		        	viewBtn= '<button type="button" class="common-btn btn-sm" id="viewbtn" onclick=OpenEntryPG("view",'+data[i].rfqId+')>View</button>';
		        	editBtn= '<button type="button" class="common-btn gray-btn btn-sm" id="editbtn" onclick=OpenEntryPG("edit",'+data[i].rfqId+')>Edit</button>';
		        	$('#tdata').append(
		        		'<tr>'+
		        		'<td hidden>'+data[i].rfqId +'</td>'+
	...
	function OpenEntryPG(sMode, rfQId) {
		srchData = {
			"searchVarData" : sMode,
			"rfQId" : rfQId
		};
