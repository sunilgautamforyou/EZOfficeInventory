...
//			+ "		or DESCRIPTION like '%' #{prjSearch.searchVarData} '%')")
	@Select("select\r\n"
			...
			+ "	coalesce(A.GSTPCT, 0)GSTPCT,\r\n"
			+ "	coalesce(A.ITEMRATE, 0)ITEMRATE,\r\n"
			+ "	coalesce(X.BalStkQty,0)BalStkQty\r\n"
			+ "from\r\n"
			+ "	ITEMMASTER A inner join\r\n"
			+ "	UOMMASTER B on A.UOMID = B.UOM_ID inner join \r\n"
			+ "	CATEGORYMASTER C on C.CATEGORYID = A.CATEGORYID \r\n"
			+ "	left outer join \r\n"
			+ "	(\r\n"
			+ "	select\r\n"
			+ "		X.ITEMID,\r\n"
			...
			+ "	 X.ITEMID = A.ITEMID\r\n"
			+ "	and X.UOMID = A.UOMID\r\n"
			+ "	where (A.ITEMNAME like '%' #{prjSearch.searchVarData} '%' \r\n"
			+ "		or DESCRIPTION like '%' #{prjSearch.searchVarData} '%')")
	@Results({
		@Result(property = "itemId",column = "ItemID"),
		@Result(property = "itemShortNm",column = "ItemName"),
...
//			+ "AND RFQID=#{rfQId}")
	...
	@Select("SELECT A.CATEGORYNAME,A.CATEGORYID,A.ITEMID,A.ITEMNAME,A.UOM, A.OPENING_QTY,A.MRNQTY, A.SALEQUANTITY , (A.OPENING_QTY + A.MRNQTY) - A.SALEQUANTITY CLOSINGQUANTITY \r\n"
		...
		@Select("SELECT A.FLATNO,\r\n"
				...
				+ "and date_format(RM.PYMNTDATE,'%Y-%m-%d') between #{prjSrchDat.fromDate} AND #{prjSrchDat.toDate} And ROUND(RM.PYMNTAMOUNT,2) > 0) A\r\n"
...
//				+ "	CATEGORYCODE,ITEMNAME")
		@Select("select\r\n"
				...
				+ "	LAST_PURCHASE_RATE,\r\n"
				+ "	LAST_PURCHASE_DATE,\r\n"
				+ "	ROUND(ROUND((MRN_QTY-SO_QTY), 2) * LAST_PURCHASE_RATE, 2) APPROX_BAL_AMT\r\n"
				+ "from\r\n"
				+ "	(\r\n"
				+ "	select\r\n"
				+ "		item.ITEMID,\r\n"
				...
				+ "		coalesce(So.SO_Amount, 0)So_Amount,\r\n"
				+ "		coalesce(itemrate.LAST_PURCHASE_RATE, 0)LAST_PURCHASE_RATE,\r\n"
				+ "		date_format(itemrate.LAST_PURCHASE_DATE, '%d-%b-%y')LAST_PURCHASE_DATE,\r\n"
				+ "		cat.CATEGORYNAME,\r\n"
				+ "		cat.CATEGORYCODE\r\n"
				+ "	from\r\n"
				+ "		ITEMMASTER item\r\n"
				...
				+ "			and PO.PODATE = X.LAST_PURCHASE_DATE)itemrate on\r\n"
				+ "		itemrate.ITEMID = item.ITEMID\r\n"
				+ "		and itemrate.UOMID = item.UOMID )X\r\n"
				+ "where\r\n"
				+ "	(X.PO_QTY > 0\r\n"
				+ "		or X.MRN_QTY > 0\r\n"
				+ "		or X.SO_QTY > 0)\r\n"
