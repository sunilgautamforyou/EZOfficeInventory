...
<h3>SALES ORDER STOCK REPORT</h3>
...
<tr>
<td class="textclr">Customer</td>
<td>:</td>
<td><h1>${stkRpt.getCustomerName()}</h1></td>
</tr>
<tr>
<td class="textclr">
Tower No
...
<td>:</td>
<td>${stkRpt.getCustomerMobileNo()}</td>
</tr>
</table>
</div>
...
<table class="text-section" style="line-height: 23px; text-align: left; float:right; padding-right: 20px;">
  ...
  <td>:</td>
  <td><h1>${stkRpt.getFromDate()}</h1></td>
  </tr>
    <tr>
  <td class="textclr1">
  Date To:
    ...
    <table class="mr0">
        ...
        <tr>
            ...
            <td style="color: #333; background-color: #ccc;">
               ...
               <input type="hidden" id="hdFrmDt" value="${stkRpt.getFromDate()}">
                <input type="hidden" id="hdToDt" value="${stkRpt.getToDate()}">
                <input type="hidden" id="flatNo" value="${stkRpt.getFlatNo()}">
                <input type="hidden" id="hdcustomerId" value="${stkRpt.getCustomerId()}">
            </td>
        </tr>
...
function getStkReportData()
{
	...
	$.ajax({
	    		...
	    		contentType: 'application/json',	
	    		   data: JSON.stringify(
	    		   	{
	    		   		"towerNo":${stkRpt.getTowerNo()},
	    		   		"flatNo":$('#flatNo').val(),
					    "fromDate":$('#hdFrmDt').val(),
					    "toDate":$('#hdToDt').val(),
	    		   	...
	    		   	success: function (data) {
	    		   		...
	    		   		if(data.length!=0) {
	    					...
	    					$('#lblSubTotal').html(formatNumber(roundToTwo(totalWithOutTaxAmount)));
	    					$('#lblTaxAmount').html(formatNumber(roundToTwo(totaltaxAmount)));
	    					$('#netAmount').html(formatNumber(Math.ceil(netAmount)));	    		   			
...
/* function getPDF(){
	...
	html2canvas($(".canvas_div_pdf")[0],{allowTaint:true}).then(function(canvas) {
	    ...
	    pdf.save("SOStockReport_From:"+$('#hdFrmDt').val()+"To:"+$('#hdToDt').val()+".pdf");
		//window.close();
    });
};
 */
